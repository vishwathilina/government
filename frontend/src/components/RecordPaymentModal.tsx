...
export default function RecordPaymentModal({
  ...
  const [selectedBill, setSelectedBill] = useState<BillSearchResult | null>(
    null
  );
  ...
  const OVERPAYMENT_TOLERANCE = 0.1; // 10%
  ...
  const searchBills = useCallback(async (query: string) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/v1/bills?search=${encodeURIComponent(
          query
        )}&status=UNPAID&limit=10`
      );
      if (response.ok) {
  ...
  const checkTransactionRef = useCallback(async (ref: string) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/v1/payments/search/transaction/${encodeURIComponent(
          ref
        )}`
      );
      if (response.ok) {
  ...
  const validateForm = (): boolean => {
    ...
    if (
      selectedBill &&
      amount > selectedBill.outstanding * (1 + OVERPAYMENT_TOLERANCE) &&
      selectedBill.outstanding > 0
    ) {
      newErrors.amount = `Payment exceeds maximum allowed (${formatCurrency(
        selectedBill.outstanding * (1 + OVERPAYMENT_TOLERANCE)
      )})`;
    }

    if (
      methodRequiresRef() &&
      (!transactionRef || transactionRef.trim().length === 0)
    ) {
  ...
  const getAmountMessage = () => {
    ...
    if (amount > outstanding && outstanding > 0) {
      ...
      return {
        type: "warning",
        message: `Overpayment of ${formatCurrency(
          overpayment
        )} will be recorded`,
      };
    }
    ...
    if (amount < outstanding && amount > 0) {
      ...
      return {
        type: "info",
        message: `Partial payment - ${formatCurrency(
          remaining
